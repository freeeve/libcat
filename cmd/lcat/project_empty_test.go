...
func TestProjectRefusesToEmptyTheSiteOnAProviderTypo(t *testing.T) {
	...
	err := projectCatalog(projectOptions{CatalogPath: nq, Providers: []string{"overdrive"}, Out: out, SimilarLimit: DefaultSimilarLimit})
	if err == nil {
...
func TestAllowEmptyWritesTheEmptyCatalog(t *testing.T) {
	nq := catalogWith(t, "marc")
	out, sentinel := sentinelOut(t)
	if err := projectCatalog(projectOptions{CatalogPath: nq, Providers: []string{"overdrive"}, Out: out, AllowEmpty: true, SimilarLimit: DefaultSimilarLimit}); err != nil {
...
func TestProjectOnAFeedlessCatalogSaysSo(t *testing.T) {
	...
	err := projectCatalog(projectOptions{CatalogPath: path, Providers: []string{"marc"}, Out: t.TempDir(), SimilarLimit: DefaultSimilarLimit})
	if err == nil {
...
func TestProjectWarnsButProceedsWhenOnlySomeFeedsAreMissing(t *testing.T) {
	nq := catalogWith(t, "marc")
	out := t.TempDir()
	if err := projectCatalog(projectOptions{CatalogPath: nq, Providers: []string{"marc", "overdrve"}, Out: out, SimilarLimit: DefaultSimilarLimit}); err != nil {
...
func TestNoProvidersKeepsItsOriginalError(t *testing.T) {
	nq := catalogWith(t, "marc")
	err := projectCatalog(projectOptions{CatalogPath: nq, Out: t.TempDir(), SimilarLimit: DefaultSimilarLimit})
