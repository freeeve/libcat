...
// TestBiblioCommonsCircuitBreaksOnUnreachable pins the fast-fail (task 469):
// an unresolvable host aborts after a bounded number of consecutive
// connection failures, naming the host, not after every driver term.
func TestBiblioCommonsCircuitBreaksOnUnreachable(t *testing.T) {
	doer := &dnsDoer{}
...
// TestBiblioCommonsCircuitBreaksOnWholesaleRejection pins the second guard
// (task 471): a host that 404s every term -- never a connection error -- still
// aborts fast with ErrPeerRejected, naming the host and status.
func TestBiblioCommonsCircuitBreaksOnWholesaleRejection(t *testing.T) {
	doer := &rejectDoer{}
