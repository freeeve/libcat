...
/** The hash for "Export these results…" (tasks/254).
 *
 ...
 *  selection with no query is refused by the server on purpose (tasks/205).
...
export function exportsHash(q: string, filters: WorkFilters, showTombstoned: boolean): string {
  ...
  // The works screen hides retired records unless asked (tasks/280), so the
