...
| `LCATD_ENRICH_TLC` (+ `_SCHEME`, `_MAX_PAGES`) | the TLC LS2 PAC peer harvest: comma-separated hosts, each a bare subdomain of `<tenant>.tlcdelivers.com` (e.g. `nbpl`) or a full vanity catalog host (e.g. `ls2pac.lapl.org`, as LAPL and many TLC libraries serve the same API); exact-prefLabel Subject-facet searches, ISBN-matched, queue-moderated, consensus-ranked ([details](authority-sources.md)) |
...
| `LCATD_ENRICH_LANGS` | the label languages the inference peer harvests (bibliocommons, tlc, sirsidynix) drive searches from, comma-separated (default `en`). Set `en,es` to also search each concept's Spanish label and reach Spanish-cataloged peers (the Chilean/LatAm SirsiDynix cluster, Miami-Dade, ...); a Spanish match still maps back to the concept's URI. Vega is English-only regardless (its homoit-concept model is English-labeled) |
