...
| **Physical ILS -- SIP2** | physical | `proxied` (lcatd bridge; SIP2 is raw TCP, no direct exists) | server-side login (93/94) | yes, bridge POST <=50 ids | bridge is CORS-open | `locations[]`: location, call number, status rollup (01-13), due date, hold queue | [V] client + bridge tested against a scripted ACS; verify against your ILS's SIP2 dialect **[?]** |
