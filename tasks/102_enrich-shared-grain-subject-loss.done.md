...
`RunEnrich` now collects every batch's results first, groups them by grain
path, and writes each grain exactly once. The rewrite
(`replaceGrainEnrichment`) also honors the "Works absent from the result are
left untouched" contract for co-grained Works: the new graph is the fresh
statements for returned Works plus the preserved subject links of absent
sibling Works and the term descriptions those links still reference (fresh
re-descriptions of a term win over stale ones). New
`TestRunEnrichSharedGrain` covers enrich-both, selective re-run preserving the
sibling, stale-term replacement, and idempotency.
