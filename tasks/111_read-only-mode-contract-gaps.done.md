...
Backend: `batch.Run` audits (and fires the grains-changed trigger) only when
`result.Applied > 0`, matching the single-record ops path whose audit rides on
a successful grain write. Verified against a fresh `LCATD_READ_ONLY=1`
instance on :8491: `POST /v1/batch/ops {"dryRun":false,...}` returns normally
and `GET /v1/audit` stays empty.

SPA: write affordances now hide behind `isReadOnly()` across the surfaces that
were missing it -- WorkEditor split bar, VisibilityPanel actions,
AuthorityEditor save/merge, ItemsPanel row/save/template/bulk controls,
Duplicates merge (button + `m` key), CopyCat target/profile CRUD, `.mrc`
staging, new-record link, auto-stage, batch delete, CopycatResults stage
(button + Enter), CopycatReview commit/revert (buttons + `c` key), BatchOps
execute and saved-query persistence, and the macro surfaces (MacroBar record,
Macros screen create/edit/delete -- found beyond the original list).
`svelte-check` clean; all 176 UI tests pass.
