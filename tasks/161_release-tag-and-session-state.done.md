...
**v0.24.0 released** (root + hugo + backend lockstep; tags verified on origin
via the release script's ls-remote guard; both Go modules confirmed
resolvable at the tag with GOPROXY=direct). The release also carries this
session's 162/163/159 work and the duplicates-language/homosaurus-v5 fixes.
Pre-release fix: root go.mod's roaringrange requirement bumped from v0.27.0
to the published v0.29.0 -- the local `replace ../roaringrange` is invisible
to tag consumers, and the module was verified to build and pass tests against
the published pin with the replace dropped. release.sh now ignores untracked
files (cross-session task files stay uncommitted by convention). queerbooks
can bump its hugo module + root pins to v0.24.0 and adopt their 014 Part 2.
