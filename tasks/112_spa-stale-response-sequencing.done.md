...
New `lib/sequence.ts` exposes `sequencer()`: each fetch takes a ticket and
applies its response only if the ticket is still the latest (`if (t.stale)
return` after the await, including catch/finally so a stale call cannot
clobber error or loading state). Applied to WorkSearch (`search` and
`loadMore`), VocabPicker, TagInput, CommandPalette, and MarcPreviewPane's
debounced `refresh`. `sequence.test.ts` covers freshness, supersession,
out-of-order async completions, and per-sequencer scoping.
