...
- AuthorityEditor: a `ConflictError` no longer reloads over the typed rows.
  It enters a `conflicted` state (Save locked) with an explicit choice:
  "Save mine anyway" refreshes only the etag and retries the save over the
  other session's change; "Discard mine and reload" is the old behavior, now
  opt-in.
- ItemsPanel: an executing bulk add refuses while `dirty` ("save (or remove)
  the pending item edits before bulk adding"); the dry-run preview is
  unaffected since it never refetches.
